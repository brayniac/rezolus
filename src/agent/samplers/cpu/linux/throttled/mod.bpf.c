...
// Start times for throttling events - more efficient as an array than hash
struct {
    __uint(type, BPF_MAP_TYPE_ARRAY);
    __uint(map_flags, BPF_F_MMAPABLE);
    __type(key, u32);
...
// Common function to handle throttling start
static void handle_throttle_start_common(u32 cgroup_id) {
...
// Common function to handle throttling end
static void handle_throttle_end_common(u32 cgroup_id) {
    if (!cgroup_id || cgroup_id >= MAX_CGROUPS) {
        return;
    }
...
int BPF_KPROBE(cpu_cfs_throttle_enter, struct task_struct *p)
{
    ...
    // Common throttle start handling
    handle_throttle_start_common(cgroup_id);
...
{
    u32 cgroup_id = get_cgroup_id_from_task(p);
    handle_throttle_end_common(cgroup_id);
    return 0;
}
...
{
    u32 cgroup_id = get_cgroup_id_from_tg(tg);
    handle_throttle_end_common(cgroup_id);
    return 0;
}
