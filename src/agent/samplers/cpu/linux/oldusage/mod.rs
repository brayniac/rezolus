...
const NAME: &str = "cpu_oldusage";

mod bpf {
    include!(concat!(env!("OUT_DIR"), "/cpu_oldusage.bpf.rs"));
}
...
fn set_name(id: usize, name: String) {
    if !name.is_empty() {
        CGROUP_CPU_OLDUSAGE_USER.insert_metadata(id, "name".to_string(), name.clone());
        CGROUP_CPU_OLDUSAGE_NICE.insert_metadata(id, "name".to_string(), name.clone());
        CGROUP_CPU_OLDUSAGE_SYSTEM.insert_metadata(id, "name".to_string(), name.clone());
        CGROUP_CPU_OLDUSAGE_SOFTIRQ.insert_metadata(id, "name".to_string(), name.clone());
        CGROUP_CPU_OLDUSAGE_IRQ.insert_metadata(id, "name".to_string(), name.clone());
        CGROUP_CPU_OLDUSAGE_STEAL.insert_metadata(id, "name".to_string(), name.clone());
        CGROUP_CPU_OLDUSAGE_GUEST.insert_metadata(id, "name".to_string(), name.clone());
        CGROUP_CPU_OLDUSAGE_GUEST_NICE.insert_metadata(id, "name".to_string(), name);
    }
}
...
fn init(config: Arc<Config>) -> SamplerResult {
    ...
    let cpu_usage = vec![
        ...
        &CPU_OLDUSAGE_NICE,
        &CPU_OLDUSAGE_SYSTEM,
        &CPU_OLDUSAGE_SOFTIRQ,
        &CPU_OLDUSAGE_IRQ,
        &CPU_OLDUSAGE_STEAL,
        &CPU_OLDUSAGE_GUEST,
        &CPU_OLDUSAGE_GUEST_NICE,
    ];
