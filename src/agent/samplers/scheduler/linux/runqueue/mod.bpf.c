...
int handle__sched_switch(u64* ctx) {
    ...
    if (bpf_core_field_exists(prev->sched_task_group)) {
        ...
        if (read_cgroup_info(prev, &cginfo) == 0) {
            cgroup_id = cginfo.id;
            ...
            if (is_new_cgroup(&cgroup_serial_numbers, cgroup_id, serial_nr)) {
                ...
                zero_cgroup_counter(&cgroup_ivcsw, cgroup_id);
                zero_cgroup_counter(&cgroup_runq_wait, cgroup_id);
                zero_cgroup_counter(&cgroup_offcpu, cgroup_id);
    ...
    if (bpf_core_field_exists(next->sched_task_group)) {
        ...
        if (read_cgroup_info(next, &cginfo) == 0) {
            cgroup_id = cginfo.id;
