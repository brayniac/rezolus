...
#[derive(Parser)]
#[command(version)]
#[command(about = "An tool for recording Rezolus metrics to a ringbuffer for instant lookback", long_about = None)]
struct Config {
...
impl Config {
    ...
    /// Get the duration for time-limited run.
    ...
    /// Get a temporary file in the destination path
    fn temporary(&self) -> std::fs::File {
