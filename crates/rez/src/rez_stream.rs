...
mod tests {
    ...
    fn stagger_is_deterministic() {
        // The hash itself is pinned beside the function it belongs to, in
        // `seal_policy`. What this covers is the half that needs a writer:
        // that the stagger actually reaches the segment boundaries.
        const MAX_ROWS: usize = 256;
        let dir = tempfile::tempdir().unwrap();
